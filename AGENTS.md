...
- **Tier-1 LLM (OpenRouter + Ollama):** **OpenRouter is chosen first** when `OPENROUTER_API_KEY` + `OPENROUTER_MODEL` are set (`OPENROUTER_TIER1_PREFERRED` defaults to `true`). Set `OPENROUTER_TIER1_PREFERRED=false` to prefer local Ollama when both exist. Tenant text features (dashboard brief, weekly report, planning narrative, etc.) use OpenRouter first via `generate_text_for_tenant` when `OPENROUTER_TENANT_TEXT_ENABLED=true` (default), with Gemini as optional fallback. Logs: filter for `openrouter_chat_completion` / `openrouter_request_failed`. See `docs/OPENROUTER.md`. Set `GEMINI_ENABLED=true` and `GEMINI_API_KEY` for Gemini-only paths (document extraction multimodal, etc.).
