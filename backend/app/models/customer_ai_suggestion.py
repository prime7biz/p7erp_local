"""Persisted AI suggestion batches and per-field items for customer AI traceability."""
...
class CustomerAiSuggestionBatch(Base):
    __tablename__ = "customer_ai_suggestion_batches"
...
class CustomerAiSuggestionItem(Base):
    __tablename__ = "customer_ai_suggestion_items"
    __table_args__ = (UniqueConstraint("batch_id", "field_key", name="uq_customer_ai_suggestion_item_batch_field"),)

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    batch_id: Mapped[int] = mapped_column(
        ForeignKey("customer_ai_suggestion_batches.id", ondelete="CASCADE"), nullable=False, index=True
    )
    tenant_id: Mapped[int] = mapped_column(
        ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False, index=True
    )
