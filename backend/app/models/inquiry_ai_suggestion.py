"""Persisted AI suggestion batches for merchandising inquiries."""
...
class InquiryAiSuggestionBatch(Base):
    __tablename__ = "inquiry_ai_suggestion_batches"
...
class InquiryAiSuggestionItem(Base):
    __tablename__ = "inquiry_ai_suggestion_items"
    __table_args__ = (UniqueConstraint("batch_id", "field_key", name="uq_inquiry_ai_suggestion_item_batch_field"),)

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    batch_id: Mapped[int] = mapped_column(
        ForeignKey("inquiry_ai_suggestion_batches.id", ondelete="CASCADE"), nullable=False, index=True
    )
    tenant_id: Mapped[int] = mapped_column(
        ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False, index=True
    )
