"""Persisted AI suggestion batches for supplier (vendor) master AI."""
...
class VendorAiSuggestionBatch(Base):
    __tablename__ = "vendor_ai_suggestion_batches"
...
class VendorAiSuggestionItem(Base):
    __tablename__ = "vendor_ai_suggestion_items"
    __table_args__ = (UniqueConstraint("batch_id", "field_key", name="uq_vendor_ai_suggestion_item_batch_field"),)

    id: Mapped[int] = mapped_column(primary_key=True, autoincrement=True)
    batch_id: Mapped[int] = mapped_column(
        ForeignKey("vendor_ai_suggestion_batches.id", ondelete="CASCADE"), nullable=False, index=True
    )
    tenant_id: Mapped[int] = mapped_column(
        ForeignKey("tenants.id", ondelete="CASCADE"), nullable=False, index=True
    )
