"""RBAC for customer-scoped AI actions (Role.permissions JSON; missing key => allow)."""
...
_CAP_PERMISSION_KEY: dict[CustomerAiCapability, str] = {
    "extract": "customers.ai.extract",
    "enrich": "customers.ai.enrich",
    "validate": "customers.ai.validate",
    "dedupe": "customers.ai.dedupe",
    "summary": "customers.ai.summary",
    "next_actions": "customers.ai.next_actions",
    "nl_search": "customers.ai.nl_search",
    "audit": "customers.ai.audit",
    "apply_suggestions": "customers.ai.apply_suggestions",
    "discard_suggestions": "customers.ai.discard_suggestions",
}
...
async def require_customer_ai_capability(db: AsyncSession, user: User, capability: CustomerAiCapability) -> None:
    ...
    scope = perms.get("customers.ai")
    if isinstance(scope, dict):
        nested = scope.get(capability)
        if nested is not None and not _permission_truthy(nested):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={"code": "AI_CAPABILITY_DENIED", "message": "This AI action is disabled for your role."},
            )
    ...
    if isinstance(scope, dict) and "enabled" in scope and not _permission_truthy(scope.get("enabled")):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "AI_CAPABILITY_DENIED", "message": "Customer AI is disabled for your role."},
        )
