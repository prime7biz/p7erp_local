"""RBAC for inquiry AI — Role.permissions JSON; missing key => allow."""
...
_CAP_PERMISSION_KEY: dict[InquiryAiCapability, str] = {
    "extract": "inquiries.ai.extract",
    "enrich": "inquiries.ai.enrich",
    "validate": "inquiries.ai.validate",
    "dedupe": "inquiries.ai.dedupe",
    "summary": "inquiries.ai.summary",
    "next_actions": "inquiries.ai.next_actions",
    "audit": "inquiries.ai.audit",
    "apply_suggestions": "inquiries.ai.apply_suggestions",
    "discard_suggestions": "inquiries.ai.discard_suggestions",
}
...
async def require_inquiry_ai_capability(db: AsyncSession, user: User, capability: InquiryAiCapability) -> None:
    ...
    scope = perms.get("inquiries.ai")
    if isinstance(scope, dict):
        nested = scope.get(capability)
        if nested is not None and not _permission_truthy(nested):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={"code": "AI_CAPABILITY_DENIED", "message": "This inquiry AI action is disabled for your role."},
            )
    ...
    if isinstance(scope, dict) and "enabled" in scope and not _permission_truthy(scope.get("enabled")):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "AI_CAPABILITY_DENIED", "message": "Inquiry AI is disabled for your role."},
        )
