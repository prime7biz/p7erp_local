"""RBAC for quotation AI — Role.permissions JSON; missing key => allow."""
...
_CAP_PERMISSION_KEY: dict[QuotationAiCapability, str] = {
    "extract": "quotations.ai.extract",
    "enrich": "quotations.ai.enrich",
    "validate": "quotations.ai.validate",
    "dedupe": "quotations.ai.dedupe",
    "summary": "quotations.ai.summary",
    "next_actions": "quotations.ai.next_actions",
...
async def require_quotation_ai_capability(db: AsyncSession, user: User, capability: QuotationAiCapability) -> None:
    ...
    scope = perms.get("quotations.ai")
    if isinstance(scope, dict):
        nested = scope.get(capability)
        if nested is not None and not _permission_truthy(nested):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
                detail={"code": "AI_CAPABILITY_DENIED", "message": "This quotation AI action is disabled for your role."},
            )
    ...
    if isinstance(scope, dict) and "enabled" in scope and not _permission_truthy(scope.get("enabled")):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "AI_CAPABILITY_DENIED", "message": "Quotation AI is disabled for your role."},
        )
