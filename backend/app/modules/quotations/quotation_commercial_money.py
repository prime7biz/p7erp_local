"""Parsing and validation for quotation commercial money and header FX (governance foundation).

Line-level FX consolidation is out of scope here; this module enforces safe numeric parsing
for rollups and consistent header FX rules when document vs buyer-target currencies differ.
"""
  ...
  if other_costs:
    for i, row in enumerate(other_costs):
      if not other_cost_row_is_persisted_for_rollup(row):
        continue
      amt = getattr(row, "calculated_amount", None) or getattr(row, "total_amount", None)
      try:
