...
import { api, ApiError, type InquiryAiAuditEntry } from "@/api/client";
...
function countLine(row: InquiryAiAuditEntry): string | null {
  const parts: string[] = [];
...
export function InquiryAiAuditHistory({ inquiryId }: Props) {
  const [open, setOpen] = useState(false);
  const [items, setItems] = useState<InquiryAiAuditEntry[] | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(null);
    try {
      const res = await api.inquiryAiAuditLog({ inquiry_id: inquiryId, limit: 40 });
      setItems(res.items);
    } catch (e) {
      logApiError("InquiryAiAuditHistory.load", e);
      setItems([]);
    ...
    <div className="mt-3 border-t border-border-subtle pt-3">
      ...
      >
        <span className="inline-flex items-center gap-2">
          ...
          Recent AI activity (this inquiry)
        </span>
      ...
      {open ? (
        <div className="mt-2 max-h-64 space-y-2 overflow-y-auto rounded-lg border border-border-subtle bg-surface-subtle/30 px-3 py-2 text-xs text-text-secondary">
          ...
          {!loading && !loadError && items && items.length === 0 ? (
            <div className="text-text-muted">No AI actions recorded for this inquiry yet.</div>
          ) : null}
          {!loading && !loadError && items && items.length > 0
            ? items.map((row) => {
