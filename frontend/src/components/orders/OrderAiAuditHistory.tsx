...
import { api, ApiError, type OrderAiAuditEntry } from "@/api/client";
...
function countLine(row: OrderAiAuditEntry): string | null {
  const parts: string[] = [];
...
export function OrderAiAuditHistory({ orderId, mode = "all" }: Props) {
  const [open, setOpen] = useState(false);
  const [items, setItems] = useState<OrderAiAuditEntry[] | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res =
        mode === "planning"
          ? await api.orderAiPlanningAuditLog({ order_id: orderId, limit: 40 })
          : mode === "simulation"
            ? await api.orderAiSimulationAuditLog({ order_id: orderId, limit: 40 })
            : await api.orderAiAuditLog({ order_id: orderId, limit: 40 });
      setItems(res.items);
    } catch (e) {
      logApiError("OrderAiAuditHistory.load", e);
      setItems([]);
    ...
    <div className="mt-3 border-t border-border-subtle pt-3">
      ...
      >
        <span className="inline-flex items-center gap-2">
          ...
          {mode === "planning"
            ? "Recent planning AI activity"
            : mode === "simulation"
              ? "Simulation & what-if audit (this order)"
              : "Recent AI activity (this order)"}
        </span>
      ...
      {open ? (
        <div className="mt-2 max-h-64 space-y-2 overflow-y-auto rounded-lg border border-border-subtle bg-surface-subtle/30 px-3 py-2 text-xs text-text-secondary">
          ...
          {!loading && !loadError && items && items.length === 0 ? (
            <div className="text-text-muted">No AI actions recorded for this order yet.</div>
          ) : null}
          {!loading && !loadError && items && items.length > 0
            ? items.map((row) => {
