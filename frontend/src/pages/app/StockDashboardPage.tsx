...
import { api, type MaterialControlStockMovementRow, type StockDashboardResponse } from "@/api/client";
...
export function StockDashboardPage() {
  ...
  const [materialTrace, setMaterialTrace] = useState<MaterialControlStockMovementRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [dash, mc] = await Promise.all([
        api.getStockDashboard({ low_stock_threshold: threshold }),
        api.listMaterialControlStockMovements({ limit: 25 }).catch((e) => {
          logApiError("StockDashboardPage.listMaterialControlStockMovements", e);
          return [];
        }),
      ]);
      setData(dash);
    ...
    <div className="min-w-0 space-y-6">
      ...
      {data && !loading && (
        <>
          ...
          <div>
            <h2 className="mb-2 text-lg font-semibold text-text-primary">Recent material-control movements</h2>
            <p className="mb-2 text-xs text-text-muted">
              Latest stock movements that carry <code className="rounded bg-surface-subtle px-1">movement_kind</code> (e.g. PMI / consumption). Filter by order in{" "}
              <Link className="text-status-info hover:underline" to="/app/inventory/stock-ledger">
                Stock ledger
              </Link>
              .
            ...
            <div className={`rounded-xl border border-border bg-surface-raised ${inventoryScrollTableClass}`}>
              <table className="min-w-[880px] w-full">
                <thead className="bg-surface-subtle">
                  <tr>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">Date</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">Kind</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">Type</th>
                    <th className="px-3 py-2 text-right text-xs font-medium uppercase text-text-muted">Qty</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">Order</th>
                    <th className="px-3 py-2 text-left text-xs font-medium uppercase text-text-muted">Ref</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-200">
                  {materialTrace.length === 0 ? (
                    <tr>
                      <td colSpan={6} className="px-3 py-6 text-center text-sm text-text-muted">
                        No traceable movements returned (or API unavailable).
                      </td>
                    </tr>
                  ) : (
                    materialTrace.map((row) => (
                      <tr key={row.id}>
                        <td className="px-3 py-2 text-sm">
                          {row.movement_date ? new Date(row.movement_date).toLocaleDateString() : "—"}
                        </td>
                        <td className="px-3 py-2 font-mono text-xs">{row.movement_kind ?? "—"}</td>
                        <td className="px-3 py-2 text-sm">{row.movement_type}</td>
                        <td className="px-3 py-2 text-right text-sm">{row.quantity}</td>
                        <td className="px-3 py-2 text-sm">{row.order_id ?? "—"}</td>
                        <td className="px-3 py-2 text-sm text-text-secondary">
                          {row.reference_type ?? "—"} {row.reference_id ?? ""}
                        </td>
                      </tr>
