...
import { api, type ProformaInvoiceRow } from "@/api/client";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-gray-200 bg-white overflow-hidden">
        {loading ? (
          <div className="p-12 text-center text-gray-500">Loading proforma invoices…</div>
        ) : items.length === 0 ? (
            ...
            <tbody>
              {items.map((row) => (
                <tr key={row.id} className="border-b border-gray-100 last:border-0">
                  <td className="py-2 px-4 font-medium text-gray-900">{row.reference ?? row.invoice_number ?? `#${row.id}`}</td>
                  <td className="py-2 px-4 text-gray-700">{row.status ?? "—"}</td>
                  <td className="py-2 px-4 text-right text-gray-700">
                    {row.amount != null ? Number(row.amount).toLocaleString() : "—"}
                  </td>
                  <td className="py-2 px-4 text-gray-700">{row.order_id != null ? row.order_id : "—"}</td>
