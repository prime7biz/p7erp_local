...
export function FinancierResetPasswordPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      setDone(true);
      setTimeout(() => navigate("/portal/financier/login", { replace: true }), 2500);
    } catch (err) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-surface-base px-4">
      <div className="w-full max-w-md rounded-2xl border border-border bg-surface-raised p-8 shadow-sm">
        ...
        <p className="text-sm text-text-muted text-center mt-1 mb-6">Financier portal</p>
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label htmlFor="portal-financier-np" className="text-xs font-medium text-text-muted">
                New password
              </label>
              <PasswordFieldInput
                id="portal-financier-np"
                value={password}
            ...
            <div>
              <label htmlFor="portal-financier-npc" className="text-xs font-medium text-text-muted">
                Confirm password
              </label>
              <PasswordFieldInput
                id="portal-financier-npc"
